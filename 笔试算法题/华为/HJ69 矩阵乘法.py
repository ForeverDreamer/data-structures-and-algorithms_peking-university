...
def execute(a, b):
    for row in [[str(sum(i * j for i, j in zip(row, col))) for col in zip(*b)] for row in a]:
        print(' '.join(row))
    # for row in a:
    #     for col in zip(*b):
...
def matrix_multiplication(seq):
    ...
    while i < len(seq):
        ...
        # z = int(seq[i+2])
        a = []
        for j in range(i+3, i+3+x):
            a.append([int(n) for n in seq[j].split(' ')])
        b = []
        for j in range(i+3+x, i+3+x+y):
            b.append([int(n) for n in seq[j].split(' ')])
        execute(a, b)
        i += 3+x+y


matrix_multiplication(input_seq)

    ...
    def isMatch(self, s: str, p: str) -> bool:
        ...
        for i in range(m+1):
            for j in range(1, n+1):
                ...
                if p[j-1] == '*':
                    # 默认匹配0次，丢弃pattern中的两个字符
                    f[i][j] = f[i][j-2]
                    if matches(i, j-1):
                        # f[i-1][j]表示当前状态也可用待匹配字符串消除最后一个字符之后，跟当前pattern的匹配状态表示
                        # 匹配0次或1次任一种情况为True当前就为True，f[i-1][j]
                        f[i][j] |= f[i-1][j]
                else:
                    # 当pattern当前字符不是*时，如果能与待匹配字符串的当前字符匹配，则双方都可以丢弃当前字符，状态由前一步决定
                    if matches(i, j):
                        f[i][j] = f[i-1][j-1]
                    # 否则就无法匹配，不改变False状态
        return f[m][n]
